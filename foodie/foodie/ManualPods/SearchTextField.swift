...
open class SearchTextField: HoshiTextField {
    ...
    open var theme = SearchTextFieldTheme.lightTheme() {
        didSet {
            ...
            if let placeholderColor = theme.placeholderColor {
                ...
                self.placeholderSearchLabel?.textColor = placeholderColor
            }
    ...
    fileprivate var placeholderSearchLabel: UILabel?
    ...
    fileprivate func buildplaceholderSearchLabel() {
        ...
        if let placeholderSearchLabel = placeholderSearchLabel {
            placeholderSearchLabel.font = self.font
            placeholderSearchLabel.frame = newRect
        } else {
            placeholderSearchLabel = UILabel(frame: newRect)
            placeholderSearchLabel?.font = self.font
            placeholderSearchLabel?.backgroundColor = UIColor.clear
            placeholderSearchLabel?.lineBreakMode = .byClipping
            ...
            if let placeholderColor = self.attributedPlaceholder?.attribute(NSAttributedString.Key.foregroundColor, at: 0, effectiveRange: nil) as? UIColor {
                placeholderSearchLabel?.textColor = placeholderColor
            } else {
                placeholderSearchLabel?.textColor = UIColor ( red: 0.8, green: 0.8, blue: 0.8, alpha: 1.0 )
            }
            
            self.addSubview(placeholderSearchLabel!)
        }
    }
    ...
    @objc open func textFieldDidChange() {
        ...
        if text!.isEmpty {
            ...
            self.placeholderSearchLabel?.text = ""
        } else {
            filter(forceShowAll: forceNoFiltering)
            prepareDrawTableResult()
        }
    ...
    @objc open override func textFieldDidBeginEditing() {
        super.textFieldDidBeginEditing()
        if (startVisible || startVisibleWithoutInteraction) && text!.isEmpty {
            clearResults()
            filter(forceShowAll: true)
        }
        placeholderSearchLabel?.attributedText = nil
    }
    
    @objc open override func textFieldDidEndEditing() {
    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                let subtitleFilterRange = item.subtitle != nil ? (item.subtitle! as NSString).range(of: text!, options: comparisonOptions) : NSRange(location: NSNotFound, length: 0)
    ...
    func handleInlineFiltering() {
        if let text = self.text {
            if text == "" {
                self.placeholderSearchLabel?.attributedText = nil
            } else {
                if let firstResult = filteredResults.first {
                    self.placeholderSearchLabel?.attributedText = firstResult.attributedTitle
                } else {
                    self.placeholderSearchLabel?.attributedText = nil
                }
            }
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 0.95), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 10), fontColor: UIColor.black)
    }
